...
long long arrayHistoryPairsRollMax_u(
    double z, long long v, long long n, long long* svv, long long d, long long nd, 
    ...
    long long* ppm)
{
  ...
  for (ac=0.0, i=0; i<n; i++)
    ac += ff[i];
 
  am = ac/pow((double)v,1.0/((double)n));
    ...
    for (w=0; svv[w]>2 && w<n; w++)
    {
      ...
      for (s=1; s<r-1; s++)
        for (t=0; t<r-2; t++)
        {
          ...
          for (j=0; j<y; j++)
          { 
            is = toIndexInsert(w, r, s, m, syy, iyy);
            it = toIndexInsert(w, r, t, m, syy, iyy);
            f += alngam(aa[is]+aa[it]+1.0) - alngam(aax[is]+aax[it]+1.0)
                 - alngam(bb[is]+bb[it]+1.0) + alngam(bbx[is]+bbx[it]+1.0);
            incIndex(m,syy,iyy);
          }
          f /= pow((double)(vc/r*(r-1)),1.0/((double)n));
          srchd++;
